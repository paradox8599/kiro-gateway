...
import time
import uuid
from typing import TYPE_CHECKING, AsyncGenerator, Dict, List, Optional, Any, Union
...
async def stream_kiro_to_anthropic(
    ...
    request_system: Optional[Union[str, List[Dict[str, Any]]]] = None,
    ...
    Args:
        ...
        request_system: System prompt (for token counting)
        request_tools: Tools definitions (for token counting)
    ...
    # Count input tokens from request messages, system prompt, and tools
    if request_messages:
        input_tokens = count_message_tokens(request_messages)
    if request_system:
        if isinstance(request_system, str):
            input_tokens += count_tokens(request_system)
        elif isinstance(request_system, list):
