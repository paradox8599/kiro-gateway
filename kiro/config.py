...
KIRO_GATEWAY_CREDS_DIR: str = "~/.kiro-gateway/"
KIRO_GATEWAY_CREDS_FILE: str = "~/.kiro-gateway/credentials.json"
...
def get_gateway_credentials_path() -> Path:
    """
    Get expanded path to kiro-gateway credentials file.

    Returns:
        Path object with expanded home directory
    """
    return Path(KIRO_GATEWAY_CREDS_FILE).expanduser()
...
def save_gateway_credentials(creds: list) -> None:
    ...
    Creates parent directory if needed and sets secure permissions (0600).
    ...
    fd, temp_path = tempfile.mkstemp(
        dir=parent_dir, prefix=".credentials_", suffix=".tmp"
    )
    try:
