...
class ModelResolution:
    ...
    Attributes:
        ...
        has_thinking_suffix: True if model name had -thinking suffix
    """
...
def normalize_model_name(name: str) -> tuple[str, bool]:
    """
    Normalize client model name to Kiro format and detect thinking mode suffix.
    ...
    8. claude-opus-4-6-thinking → (claude-opus-4.6, True) (thinking mode suffix)

    Args:
        name: External model name from client

    Returns:
        Tuple of (normalized_name, has_thinking_suffix)
        - normalized_name: Model name in Kiro format
        - has_thinking_suffix: True if model name ended with -thinking
    ...
    Examples:
        ...
        >>> normalize_model_name("claude-4.5-opus-thinking")
    ...
    has_thinking_suffix = name_lower.endswith("-thinking")
    ...
    if match:
        ...
        return f"{base}.{minor}", has_thinking_suffix  # claude-haiku-4.5
    ...
    if match:
        return match.group(1), has_thinking_suffix  # claude-sonnet-4
    ...
    if match:
        return match.group(1), has_thinking_suffix
    ...
    if match:
        ...
        return (
            f"claude-{family}-{major}.{minor}",
            has_thinking_suffix,
        )  # claude-opus-4.5

    # Pattern 6: Inverted format without suffix (after -thinking was stripped)
    # Matches: claude-4.5-opus, claude-4.5-sonnet, claude-4.5-haiku (version 4+)
    # Does NOT match: claude-3.7-sonnet (legacy format, handled by Pattern 3)
    # Convert to: claude-{family}-{major}.{minor}
    inverted_no_suffix_pattern = r"^claude-([4-9]\d*)\.(\d+)-(haiku|sonnet|opus)$"
    match = re.match(inverted_no_suffix_pattern, name_lower)
    if match:
        major = match.group(1)  # 4
        minor = match.group(2)  # 5
        family = match.group(3)  # opus
    ...
    return name_lower if has_thinking_suffix else name, has_thinking_suffix
...
def get_model_id_for_kiro(model_name: str, hidden_models: Dict[str, str]) -> str:
    ...
    Examples:
        ...
        'CLAUDE_3_7_SONNET_20250219_V1_0'
    """
    normalized, _ = normalize_model_name(model_name)
...
class ModelResolver:
    ...
    def resolve(self, external_model: str) -> ModelResolution:
        ...
        normalized, has_thinking = normalize_model_name(resolved_model)
        ...
        if self.cache.is_valid_model(normalized):
            ...
            return ModelResolution(
                ...
                is_verified=True,
                has_thinking_suffix=has_thinking,
            )
